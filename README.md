...
### XML files
The comprehensive text data in XML format (generated by [text_scraper.py](#text_scraperpy)) had to be uploaded to the server so as to be pushed to the database.  These XML files assigned to a directory named 'xml_data'.  The textual information (OCR) extracted by mining the PDFs has been transcribed at a high degree of detail, entailing the pixel height and coordinates of each discrete text object.  The structure of such files is as follows:
